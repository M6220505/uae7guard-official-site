 ...
 * @param {Object} web3WalletData.blockchainData - Live RPC/explorer/threat-feed intelligence
 * @returns {Promise<Object>} Comprehensive risk assessment
 */
async function analyzeWeb3Wallet(web3WalletData) {
  const {
    address,
    balance = '0',
    chainId,
  ...
  // Connected wallet UI sends a formatted native-token balance string; server-side
  // intelligence can override it with a fresh RPC balance when available.
  const balanceEth = Number.isFinite(Number(blockchainData?.balanceEth))
    ? Number(blockchainData.balanceEth)
    : parseFloat(balance) || 0;

  const liveBlockchainData = blockchainData || {
    walletAgeDays: 0,
    transactionCount: 0,
    threatScore: 0,
    ...
    dataQuality: {
      ...
      explorerApi: false,
      threatFeeds: false,
      limitations: ['No live blockchain intelligence was supplied by the API route.'],
    },
  };

  // Prepare input for risk calculation from live RPC/explorer/threat-feed reads.
  const riskInput = {
    walletAddress: address,
    walletAgeDays: liveBlockchainData.walletAgeDays,
    transactionCount: liveBlockchainData.transactionCount,
    balanceEth: balanceEth,
    threatScore: liveBlockchainData.threatScore,
    blacklistAssociations: liveBlockchainData.blacklistAssociations,
    isDirectlyBlacklisted: liveBlockchainData.isDirectlyBlacklisted,
    isSmartContract: liveBlockchainData.isSmartContract,
    transactionValue: 0, // Current transaction value (if analyzing a specific tx)
    historicalAddresses: historicalAddresses,
    contractData: liveBlockchainData.contractData || contractData,
  };

  // Calculate risk using the optimized engine
  const riskAssessment = await calculateOptimizedRisk(riskInput);
  ...
  return {
    ...
    web3Metadata: {
      ...
      dataQuality: liveBlockchainData.dataQuality,
      chainProfile: liveBlockchainData.chainProfile,
      approvals: liveBlockchainData.approvals || [],
      threatIntel: liveBlockchainData.threatIntel,
    },
  };
}

/**
 * Deterministic fixture generator kept for isolated unit tests only.
 * Production API routes now supply live RPC/explorer intelligence.
 */
